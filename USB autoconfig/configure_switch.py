...
def prompt_user():
    ...
    port = input("Enter the serial/console port (e.g., /dev/cu.usbserial-XXXX): ")
    model = input("Select the switch model (1. 9500, 2. 9300): ")
    stacked = input("Is this a stacked switch? (yes/no): ")
    uplink_description = input("Enter the description for the P2P: ")
...
interface twe1/0/23
 ...
 no switchport
 ip address {first_ptp_address} 255.255.255.254
...
if __name__ == "__main__":
    hostname, enable_password, admin_password, loopback_ip, ospf_num, tacacs_key, port, model, stacked, uplink_description, first_ptp_address, second_ptp_address, ospf_message_key = prompt_user()
    try:
